...
public class CardPaymentStrategy implements PaymentStrategy {
    private static final Set<String> SUPPORTED_PROVIDERS = Set.of("VISA", "MASTERCARD");
    ...
    public Payment processPayment(PaymentRequestDto request, String sessionReference) {
        // Performance: Prepare external API request
        ...
        try {
            // Performance: Single HTTP call to payment provider
            ResponseEntity<Map> response = restTemplate.postForEntity(
                    paymentProviderUrl + "/v1/card-payment", entity, Map.class);
            ...
            if (response.getStatusCode() == HttpStatus.OK && response.getBody() != null) {
                ...
                payment.setFailureReason("Payment provider request failed");
            }

            return paymentRepository.save(payment);

        } catch (Exception e) {
            // Performance: Fail fast with error logging
            Payment failedPayment = new Payment();
            failedPayment.setPaymentReference(request.paymentReference());
            failedPayment.setStatus(PaymentStatus.FAILED);
            failedPayment.setFailureReason("Payment processing error: " + e.getMessage());
